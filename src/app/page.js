...
export default function Home() {
  ...
  const getNetworthWithMoralis = async () => {
    try {
      ...
      const response = await fetch(`/api/moralis/${walletAddress}`, {
        method: 'GET',
      });

      const data = await response.json();
      const totalNetworth = data['netWorth']['total_networth_usd'];

      setWalletBalance(USDollar.format(totalNetworth));
    } catch (error) {
